export type ThirdPlaceDuel = 74 | 77 | 79 | 80 | 81 | 82 | 85 | 87
export type GroupWinnerSlot = "A" | "B" | "D" | "E" | "G" | "I" | "K" | "L"

const DUEL_TO_WINNER_SLOT: Record<ThirdPlaceDuel, GroupWinnerSlot> = {
  79: "A",
  85: "B",
  81: "D",
  74: "E",
...
/**
 * Source rows from the 2026 FIFA World Cup third-place combination table.
 * Row format:
 *   <COMBO> <vs1A> <vs1B> <vs1D> <vs1E> <vs1G> <vs1I> <vs1K> <vs1L>
 *
 * Example:
 *   ACDEFGHI H G E C A F D I
 */
const RAW_THIRD_PLACE_MATRIX = `
...
export const derdePlaatsingMatrix: Record<string, Record<GroupWinnerSlot, string>> =
  Object.fromEntries(
    RAW_THIRD_PLACE_MATRIX.split("\n").map((line) => {
      const [combo, a, b, d, e, g, i, k, l] = line.trim().split(/\s+/)
      return [
        combo,
        {
          A: a,
...
export const getBesteDerdeCombinatieKey = (poules: string[]) =>
  [...poules].sort().join("")

export const getDerdePouleVoorDuel = (
  combinatieKey: string,
  duel: ThirdPlaceDuel
): string | null => {
  const winnerSlot = DUEL_TO_WINNER_SLOT[duel]
  return derdePlaatsingMatrix[combinatieKey]?.[winnerSlot] ?? null
}

export const getDerdePoulesPerDuel = (combinatieKey: string) => {
  const row = derdePlaatsingMatrix[combinatieKey]
  if (!row) return null
  ...
  return {
    ...
    85: row.B,
    87: row.K,
  } as Record<ThirdPlaceDuel, string>
}